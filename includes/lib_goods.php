 ...
 * @return  array
 */
function get_goods_second_img($goods_id)
{
    $sql = 'SELECT img_id, img_url' .
        ' FROM ' . $GLOBALS['ecs']->table('goods_gallery') .
        " WHERE goods_id = '$goods_id' ORDER BY img_id ASC LIMIT 1,1";
    $row = $GLOBALS['db']->getAll($sql);
    /* 格式化相册图片路径 */
    $arr = '';
    foreach($row as $key => $gallery_img)
    {
        $arr = get_image_path($goods_id, $gallery_img['img_url'], false, 'gallery');
    }
    return $arr;
}
